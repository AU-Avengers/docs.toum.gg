---
title: Host Specific Options
description: Settings for the host to adjust.
custom_edit_url: null
---

import useBaseUrl from '@docusaurus/useBaseUrl';

<img src={useBaseUrl('/img/Icons/Monarch.png')} align="left" style={{ height: 65, marginRight: '12px' }} />

# Host Specific Options

Settings for the host to adjust limits to what the players have access to. Also allows adjusting Anti-Cheat settings for any potentially bad actors.

| Option                        |                                                   Description                                                    |  Type  | Default |    Range     |
| ----------------------------- | :--------------------------------------------------------------------------------------------------------------: | :----: | :-----: | :----------: |
| Enable Anti-Cheat Warnings    |              This will enable messages for the host to see which mods players join the lobby with.               | Toggle |  True   | True / False |
| Kick Players Using Cheat Mods |                       This will automatically kick players who join with known cheat mods.                       | Toggle |  True   | True / False |
| Freeplay Mode                 |                                    Allows for Freeplay testing functionality.                                    | Toggle |  False  | True / False |
| Allow Lobby-Only No-Clip      |                    This allows players to use the no-clip feature when in lobby (using Ctrl).                    | Toggle |  True   | True / False |
| Allow Spectators              | This allows players to use Spectator mode, allowing them to Spectate the game without having to leave the lobby. | Toggle |  True   | True / False |
